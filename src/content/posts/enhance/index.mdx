...
![](assets/image-white.png)(class:img-light)

![](assets/image-black.png)(class:img-dark)
:::
...
In order to ensure the style size on the phone, I have set a minimum width of 300px, and you can modify and expand the style in `src/styles/markdown.css`.
...
The theme provides the following one predefined badges. You can customize them via the `badge` option in `plugins/index.ts` and style them under `/* :badge */` in `src/styles/markdown.css`.
