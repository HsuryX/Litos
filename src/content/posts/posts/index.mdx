...
| &nbsp;&nbsp;&nbsp;&nbsp;type                                                | In the **read page**, the style of displaying data in a list: `compact`, `minimal`, `time-line`, `image`, or `cover`.                                                                                              |
...
| commentsText                                                                | The heading shown above the comments section on post detail pages.                                                                                                                                                 |
| wordCountView                                                               | Whether to show the word count and reading time on post detail pages.                                                                                                                                              |
...
| license     | Optional SPDX-style license identifier (e.g. `CC-BY-NC-SA-4.0`). When set, a license block is rendered at the bottom of the post.                                                       |
...
The theme has a built-in :link[Giscus]{id=https://github.com/giscus/giscus} comment system powered by GitHub Discussions. Each post automatically gets its own discussion thread.

#### Prerequisites

1. Create a **public repository** on GitHub to host the discussions (it can be the same repo as your blog source).
2. Enable the **Discussions** feature in the repository's **Settings → Features**.
3. Install the :link[giscus app]{id=https://github.com/apps/giscus} on that repository.
4. Go to :link[giscus.app]{id=https://giscus.app} and fill in your repo. The page will generate `data-repo-id` and `data-category-id` values — copy them.
...
No environment variables are required — `repoId` and `categoryId` are public identifiers that are safe to commit.
...
| system           | Comment system to use. Options: `giscus`, `none`.                                               |
| repo             | GitHub repository in `owner/repo` form that hosts the discussions.                              |
| repoId           | Repository ID generated at :link[giscus.app]{id=https://giscus.app}.                            |
| category         | Discussion category name (e.g. `General`, `Announcements`).                                     |
| categoryId       | Category ID generated at :link[giscus.app]{id=https://giscus.app}.                              |
| mapping          | Page → discussion mapping. One of `pathname`, `url`, `title`, `og:title`, `specific`, `number`. |
| strict           | `'1'` for strict title matching, `'0'` otherwise.                                               |
| reactionsEnabled | `'1'` to enable reactions on the main post.                                                     |
| emitMetadata     | `'1'` to emit discussion metadata messages to the host page.                                    |
| inputPosition    | `top` or `bottom` — position of the comment input relative to the thread.                       |
| lang             | Display language code (e.g. `en`, `zh-CN`, `ja`).                                               |
...
> [!TIP]
> The Giscus iframe theme is synced to the site's light/dark mode automatically — no extra configuration required.
